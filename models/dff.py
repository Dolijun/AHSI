...
class DFF(nn.Module):
    ...
    def __init__(self, nclass, feat_chans, norm_layer=nn.BatchNorm2d):
        ...
        self.side1 = nn.Sequential(nn.Conv2d(feat_chans[0], 1, 1),
                                   norm_layer(1))
        self.side2 = nn.Sequential(nn.Conv2d(feat_chans[0], 1, 1, bias=True),
                                   norm_layer(1),
                                   nn.ConvTranspose2d(1, 1, 4, stride=2, padding=1, bias=False))
        self.side3 = nn.Sequential(nn.Conv2d(feat_chans[1], 1, 1, bias=True),
                                   norm_layer(1),
                                   nn.ConvTranspose2d(1, 1, 8, stride=4, padding=2, bias=False))
        self.side5 = nn.Sequential(nn.Conv2d(feat_chans[3], nclass, 1, bias=True),
                                   norm_layer(nclass),
                                   nn.ConvTranspose2d(nclass, nclass, 16, stride=8, padding=4, bias=False))

        self.side5_w = nn.Sequential(nn.Conv2d(feat_chans[3], nclass * 4, 1, bias=True),
                                     norm_layer(nclass * 4),
                                     nn.ConvTranspose2d(nclass * 4, nclass * 4, 16, stride=8, padding=4, bias=False))

    def forward(self, input_, res1, res2, res3, res4, res5):
        if res1 is None:
        ...
        side1 = self.side1(res1)  # (N, 1, H, W)
        side2 = self.side2(res2)  # (N, 1, H, W)
        side3 = self.side3(res3)  # (N, 1, H, W)
        side5 = self.side5(res5)  # (N, 19, H, W)
        side5_w = self.side5_w(res5)  # (N, 19*4, H, W)
