...
%% S/home/ilab/dolijun/Dolijun/数据集/tianjinetup Parallel Pool
num_worker = 16; %12;
delete(gcp('nocreate'));
...
for idx_dir = 1:length(eval_dir)
    ...
    %% get road 
%     fprintf('Benchmarking boundaries for category 1: Road\n');
%     result_cls = benchmark_category(list_eval, eval_dir{idx_dir}, gt_dir, 1, margin, nthresh, thinpb, maxDist, 0);
%     save([result_dir{idx_dir} '/class_1.mat'], 'result_cls');
%     load([result_dir{idx_dir} '/class_1.mat'])
%     besT_road = result_cls{2, 1}(1);
% 
%     % 在预测为Road的基础上预测其他类别
%     num_cls = length(categories);
%     for idx_cls = 2:num_cls %%1
%         fprintf('Benchmarking boundaries for category %d: %s\n', idx_cls, categories{idx_cls});
%         result_cls = benchmark_category(list_eval, eval_dir{idx_dir}, gt_dir, idx_cls, margin, nthresh, thinpb, maxDist,besT_road);
...
%     end
   %% orig_bench
    ...
    for idx_cls = 1:num_cls %%1
        fprintf('Benchmarking boundaries for category %d: %s\n', idx_cls, categories{idx_cls});
        result_cls = benchmark_category(list_eval, eval_dir{idx_dir}, gt_dir, idx_cls, margin, nthresh, thinpb, maxDist, 0);
        save([result_dir{idx_dir} '/class_' num2str(idx_cls) '.mat'], 'result_cls');
    end
    ...
    for idx_result = 1:num_result
        ...
        load([result_dir{idx_dir} '/class_' num2str(idx_cls) '.mat'])
...
%     fprintf('\n      Mean MF-ODS:  %.3f\n\n', mean(result_f));
    ...
    fprintf(fid, '====================== Summary MF-ODS ======================\n\n');
    for idx_cls = 1:num_cls
        fprintf(fid,'%2d %14s:  %.3f\n', idx_cls, categories{idx_cls}, result_f(idx_cls));
    end
    fprintf(fid,'\n      Mean MF-ODS:  %.3f\n\n', mean(result_f));
    fclose(fid);
